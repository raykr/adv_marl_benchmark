		...
		for dirname, dirs, files in os.walk(rootdir):
			for filter in filters:
				if filter in dirname:
					result = {'dirname' : dirname.split("/")[-1].split("\\")[-1], "data": None}
	...
	if average_group:
		for group in sorted(groups):
			...
			if (ykey[0] in group or len(ykey) != 2) or not yduel:
				...
				pltt = plt2
			legend, = pltt.plot(usex, ymean, color=color, linewidth=2)
			# handle dual y label
			if ylabel is not None:
				if yduel:
					pltt.set_ylabel(current_group['ykey'])# , fontsize=15)
				else:
					pltt.set_ylabel(ylabel)# , fontsize=15)
			
			current_group['legend'] = legend
			if shaded_err:
				pltt.fill_between(usex, ymean - ystderr, ymean + ystderr, color=color, alpha=.4)
			if shaded_std:
				pltt.fill_between(usex, ymean - 2 * ystd,    ymean + 2 * ystd, color=color, alpha=.2)

	# add legend
	# https://matplotlib.org/stable/api/_as_gen/matplotlib.pyplot.legend.html
	if any(groups_results.keys()) and use_legend:
		if legend_group_num:
			plt.legend(
