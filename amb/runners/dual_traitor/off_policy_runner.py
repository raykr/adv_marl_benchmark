...
class OffPolicyRunner(BaseRunner):
    ...
    def __init__(self, args, algo_args, env_args):
        """Initialize the dual_traitor/OffPolicyRunner class.
        Args:
        ...
        if self.algo_args["angel"]['use_render'] is False:  # train, not render
            ...
            self.buffer = EpisodeBuffer(algo_args["angel"], self.algo_args["angel"]["buffer_size"], scheme, num_agents=self.num_adv_agents)
    ...
    def init_batch(self):
        ...
        data = {
            ...
            "obs": gather(obs[0], adv_agent_ids, axis=1).copy(),
            "share_obs": gather(share_obs[0], adv_agent_ids, axis=1).copy()
        }
        if "available_actions" in self.buffer.data:
            data["available_actions"] = gather(available_actions[0], adv_agent_ids, axis=1).copy()
    ...
    def run(self):
        ...
        while self.current_timestep < self.algo_args["angel"]['num_env_steps']:
            ...
            adv_rnn_states = np.zeros((self.n_rollout_threads, self.num_adv_agents, self.angel_recurrent_n, self.angel_rnn_hidden_size), dtype=np.float32)
            ...
            for step in range(self.episode_length):
                adv_actions, adv_actions_onehot, adv_rnn_states = self.collect(
                    gather(obs[0], adv_agent_ids, 1), 
                    adv_rnn_states, 
                    adv_masks, 
                    gather(available_actions[0], adv_agent_ids, 1)
                ...
                data = {
                    ...
                    "rewards": gather(rewards[0], adv_agent_ids, 1), "dones": dones[0], "infos": infos[0], 
    ...
    def insert(self, data, step):
        ...
        data["trunc_env"] = np.zeros((self.n_rollout_threads, self.num_victims, 1), dtype=np.float32)
