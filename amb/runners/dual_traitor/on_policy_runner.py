...
from amb.runners.dual_traitor.base_runner import BaseRunner
...
class OnPolicyRunner(BaseRunner):
    def __init__(self, args, algo_args, env_args):
        """Initialize the dual_traitor/OnPolicyRunner class.
        Args:
    ...
    def init_batch(self):
        ...
        for agent_id in range(self.num_adv_agents):
            data = {
                ...
                "obs": gather(obs[0], adv_agent_ids, axis=1)[:, agent_id].copy(),
                "share_obs": gather(share_obs[0], adv_agent_ids, axis=1)[:, agent_id].copy(),
            }
            if "available_actions" in self.buffers[agent_id].data:
                data["available_actions"] = gather(available_actions[0], adv_agent_ids, axis=1)[:, agent_id].copy()
    ...
    def run(self):
        ...
        victim_rnn_states = np.zeros((self.n_rollout_threads, self.num_victims, self.victim_recurrent_n, self.victim_rnn_hidden_size), dtype=np.float32)
        victim_masks = np.ones((self.n_rollout_threads, self.num_victims, 1), dtype=np.float32)
        ...
        for episode in range(1, episodes + 1):
            ...
            for step in range(self.algo_args["angel"]['episode_length']):
                ...
                victim_actions_collector = []
                for agent_id in range(self.num_victims):
                    victim_actions, temp_rnn_state = self.victims[agent_id].perform(
                        obs[0][:, agent_id],
                        victim_rnn_states[:, agent_id],
                        victim_masks[:, agent_id],
                        available_actions[0][:, agent_id]
                        if available_actions[0][0] is not None else None,
                        deterministic=False
                    )
                    victim_rnn_states[:, agent_id] = _t2n(temp_rnn_state)
                    victim_actions_collector.append(_t2n(victim_actions))
                victim_actions = np.stack(victim_actions_collector, axis=1)
                ...
                obs, share_obs, rewards, dones, infos, available_actions = self.envs.step((victim_actions, demon_actions))

                filled = np.ones((self.n_rollout_threads, self.num_adv_agents), dtype=np.float32)

                data = {
                    "obs": gather(obs[0], adv_agent_ids, 1), "share_obs": gather(share_obs[0], adv_agent_ids, 1), 
                    "rewards": gather(rewards[0], adv_agent_ids, 1),  "dones": dones[0], "adv_agent_ids": adv_agent_ids,
                    "infos": infos[0], "value_preds": adv_values, "actions": adv_actions, "action_log_probs": adv_action_log_probs,
                ...
                if "available_actions" in self.buffers[0].data:
                    data.update({"available_actions": gather(available_actions[0], adv_agent_ids, 1)})
                
                data["rewards"] = self.get_adv_rewards(data)

                dones_env = np.all(dones[1], axis=1)

                victim_rnn_states[dones_env==True] = 0
                victim_masks = np.ones((self.n_rollout_threads, self.num_victims, 1), dtype=np.float32)
                victim_masks[dones_env==True] = 0
    ...
    def insert(self, data, step):
        ...
        data["masks"] = np.ones((self.n_rollout_threads, self.num_victims, 1), dtype=np.float32)
        data["masks"][dones_env==True] = 0
        data["masks"] = gather(data["masks"], data["adv_agent_ids"], 1)

        data["active_masks"] = np.ones((self.n_rollout_threads, self.num_victims, 1), dtype=np.float32)
        ...
        data["bad_masks"] = np.ones((self.n_rollout_threads, self.num_victims, 1), dtype=np.float32)
        for i in range(self.n_rollout_threads):
